...
contract Artist is ERC1155, ERC1155Supply, AccessControl, Pausable, Ownable {
    ...
    constructor(string memory _artistName, address _artistAddress) ERC1155("https://tunebase.com/api/tokens/{id}") {
        _grantRole(DEFAULT_ADMIN_ROLE, _artistAddress);
        _grantRole(URI_SETTER_ROLE, _artistAddress);
        _grantRole(PAUSER_ROLE, _artistAddress);
        _grantRole(MINTER_ROLE, _artistAddress);
    ...
    function updateReleaseTitle(uint256 _releaseId, string memory _newTitle) public {
        releaseIdToReleases[_releaseId].title = _newTitle;
    }
    ...
    function buyRelease(uint _releaseId) public payable {
        ...
        require(msg.value >= price, "Must send enough to purchase the release.");
